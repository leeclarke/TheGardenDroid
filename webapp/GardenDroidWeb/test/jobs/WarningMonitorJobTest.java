...
public class WarningMonitorJobTest extends BaseUnitTest {
	...
	@Test
	public void checkForDisabledWarningMode() {
		options.enableWarningNotification = false;
		
		WarningMonitorJob warning =  new WarningMonitorJob();
		boolean status = warning.processAlert(options, AlertType.DROID_DOWN);
		assertTrue("Email should not have been sent because it is disabled in options.",!status);
		
		options.enableWarningNotification = true;
	}
